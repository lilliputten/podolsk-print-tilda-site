...
if [ ! -z "${PROJECT_INFO_JSON_FILE}" ]; then
  ...
  echo "{ \"appInfo\": \"$PROJECT_INFO\" }" > $rootPath/$PROJECT_INFO_JSON_FILE
fi

if [ ! -z "${PROJECT_INFO_SCSS_FILE}" ]; then
  echo "Creating '$PROJECT_INFO_SCSS_FILE' file..."
  echo -n "/*! $PROJECT_INFO */" > $rootPath/$PROJECT_INFO_SCSS_FILE
  # # UNUSED: Put css version variable...
  # echo ":root { --app-info:" > $rootPath/$PROJECT_INFO_SCSS_FILE
